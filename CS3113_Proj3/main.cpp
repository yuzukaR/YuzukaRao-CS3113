...
bool rightkey = false;
bool leftkey = false;
const float decelerationRate = 5.0f;
...
void initialise()
{
    ...
    for (int i = 0; i < PLATFORM_COUNT; i++)
    {
        ...
        if (i == 0) {
            g_game_state.platforms[i].set_position(glm::vec3(i - 2.0f, -2.0f, 0.0f));
        }
        else if (i == 1) {
            g_game_state.platforms[i].set_position(glm::vec3(i - 0.0f, -1.0f, 0.0f));
        }
        else if (i == 2) {
            g_game_state.platforms[i].set_position(glm::vec3(i + 3.0f, -2.0f, 0.0f));
        }
...
void process_input()
{
    ...
    g_game_state.player->set_leftkey(false);
    g_game_state.player->set_rightkey(false);
    ...
    if (key_state[SDL_SCANCODE_LEFT])
    {
        ...
        //leftkey = true;
        //rightkey = false;
        g_game_state.player->set_leftkey(true);
    ...
    else if (key_state[SDL_SCANCODE_RIGHT])
    {
        ...
        //rightkey = true;
        //leftkey = false;
        g_game_state.player->set_rightkey(true);
...
void update()
{
    ...
    if (g_game_state.player->get_complete()) {
        mission_complete = true;
    }
    if (g_game_state.player->get_fail())
    {
        mission_fail = true;
    }
...
void render()
{
    ...
    //DrawText(&g_shader_program, g_leftwon_texture_id, "Mission Complete", 2.0f, 1.0f, glm::vec3(0.0f, 0.0f, 0.0f));
    if (mission_complete) {
        DrawText(&g_shader_program, complete, "Mission Accomplished", 0.3f, 0.1f, glm::vec3(-3.5f, 0.0f, 0.0f));
        //draw_object(g_win_model_matrix, g_leftwon_texture_id);
    }
    if (mission_fail) {
        DrawText(&g_shader_program, complete, "Mission Failed", 0.5f, 0.1f, glm::vec3(-4.5f, 0.0f, 0.0f));
    }
